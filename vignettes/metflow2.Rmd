...
metflow2::processData(path = ".", polarity = "positive", peakwidth = c(5,30), threads = 5, output.tic = TRUE, output.bpc = TRUE, min.fraction = 0.8, output.rt.correction.plot = TRUE, fill.peaks = FALSE)
```
...
> Note:
`test.database` is only a demo database (metflow2entifyClass). We will don't use it for next metabolite identification.
...
> Note:
`test.database2` is only a demo database (metflow2entifyClass). We will don't use it for next metabolite identification.
...
The argument of `metflow2entify` can be found using `?metflow2entify`.
...
`result` is metflow2entifyClass object. You can print it out to see the identificaiton information.
...
You can use `filterIden` function to filer identification results from the `metflow2entifyClass` object according to m/z error, rt error, MS2 spectra similarity and total score.
...
Here, we will use orbitrap database and HMDB database for metabolite identification. The `metflow2entify4all` function can identify metabolites with several databases.
...
We can use `metflow2entifyParam` and `mzIdentifyParam` to get parameter list for different databases.

```{r, eval = FALSE,warning=FALSE,message=FALSE,R.options="",cache=TRUE}
parameter.list1 <- metflow2entifyParam(polarity = "positive", 
                                    candidate.num = 3, 
                                    column = "rp", ce = "all", 
                                    database = "orbitrapDatabase0.0.1", 
                                    threads = 3)

parameter.list2 <- metflow2entifyParam(polarity = "positive", 
                                    candidate.num = 3, 
                                    column = "rp", ce = "all", 
...
### Run `metflow2entify4all` function

```{r, eval = FALSE,warning=FALSE,message=FALSE,R.options="",cache=TRUE}
result2 <- metflow2entify4all(ms1.data = "ms1.peak.table.csv", ##csv format  
...
> Note:
Of course, you can also use `metflow2entify4all` function to run all the databases(in-house, publica database and HMDB metabolite database) in one run like we have shown above. What should be noticed is that the parameter list for HMDB.metabolite.data must use `mzIdentifyParam` function to construct.
